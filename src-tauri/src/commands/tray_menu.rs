...
pub fn create_bootstrap_tray_menu(app: &AppHandle) -> Result<Menu<Wry>, String> {
    MenuBuilder::new(app)
        .item(
            &MenuItem::with_id(app, BOOTSTRAP_HEADER_ID, "CodexMaMi", false, None::<&str>)
                .map_err(|e| e.to_string())?,
        )
        .item(
            &MenuItem::with_id(app, BOOTSTRAP_TITLE_ID, "CodexMaMi", true, None::<&str>)
                .map_err(|e| e.to_string())?,
        )
        .item(
            &MenuItem::with_id(
        ...
        .separator()
        .item(
            &MenuItem::with_id(app, OPEN_MAIN_ID, "Open CodexMaMi", true, None::<&str>)
                .map_err(|e| e.to_string())?,
        )
        .separator()
        .item(
